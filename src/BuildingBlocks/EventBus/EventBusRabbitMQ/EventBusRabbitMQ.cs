...
namespace Kwetter.BuildingBlocks.EventBus.EventBusRabbitMQ
{
    ...
    {
        private const string BROKER_NAME = "kwetter_event_bus";
        ...
        )
        {
            ...
            container.RegisterAssemblyTypes(typeof(int).GetTypeInfo().Assembly).AsClosedTypesOf();// Populate(services);
        ...
        private async Task ProcessEvent(string eventName, string message)
        {
            ...
            if (_subsManager.HasSubscriptionsForEvent(eventName))
            {
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    {
                        var handler = scope.ServiceProvider.GetService(subscriptionHandlerType);
                ...
                // await Task.Yield();
                // TODO maybe replace mediatr
                /* using var scope = _serviceProvider.CreateScope();
                var mediator = scope.ServiceProvider.GetRequiredService<IPublisher>();
                var mediator1 = _serviceProvider.GetService<IPublisher>();
        ...
        public void Publish(IntegrationEvent @event)
        {
            ...
            // string message = JsonSerializer.Serialize(@event, new JsonSerializerOptions() {IncludeFields = true});
            // byte[] body = JsonSerializer.SerializeToUtf8Bytes(@eventName, new JsonSerializerOptions {IncludeFields = true});
